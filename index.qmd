...
Let's say that we are looking at a Kratom shop in the real world, and the total sales are 1850. After moving from *Node 1* to *Node 2*, there's only so much else we can do. Although *Node 3* and *Node 4* can predict the violent crime rate, since these two falls under *Node 2*, they are not helpful because the p-value is greater than 0.05. This means that these results have **NO STATISTICAL SIGNIFICANCE**.
...
Suppose the shop in question's total sales were 3560, for example. In that case, we could estimate that the violent crime rate that week would be between 600 and 800, possibly even 1000.
